...
export default function PerformanceCharts({ stats }: PerformanceChartsProps) {
  ...
  const calculateStyleMetrics = () => {
    ...
    return [
      { metric: 'Agressivité', value: aggression, fullMark: 100 },
      { metric: 'Précision', value: precision, fullMark: 100 },
      { metric: 'Défense', value: defense, fullMark: 100 },
      { metric: 'Tactique', value: tactical, fullMark: 100 },
      { metric: 'Positionnel', value: positional, fullMark: 100 },
    ];
  };

  // 2. Données pour le graphique en camembert des résultats
  const resultsData = [
    { name: 'Victoires', value: stats.winRate, color: '#22c55e' },
    { name: 'Nulles', value: stats.drawRate, color: '#94a3b8' },
    { name: 'Défaites', value: stats.lossRate, color: '#ef4444' },
  ];
  ...
  const phaseData = [
    { phase: 'Ouverture', performance: 75 },
    { phase: 'Milieu', performance: stats.winRate },
    { phase: 'Finale', performance: stats.winRate + 5 },
  ];
    ...
    <Card className="bg-slate-900 border-slate-800">
      ...
      <CardContent>
        <Tabs defaultValue="style" className="w-full">
          ...
          <TabsContent value="style" className="mt-6">
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-semibold text-slate-300">Profil de Jeu</h3>
          ...
          {/* TAB 2: Distribution des Résultats */}
          <TabsContent value="results" className="mt-6">
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-semibold text-slate-300">Distribution des Résultats</h3>
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div className="bg-slate-950 p-3 rounded border border-slate-700">
                  <div className="text-center">
                    ...
                    <div className="text-xs text-slate-400 mt-1">Nulles</div>
                  </div>
                </div>
          ...
          <TabsContent value="openings" className="mt-6">
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-semibold text-slate-300">Répertoire d'Ouvertures</h3>
                ...
                <BarChart data={openingsData}>
                  ...
                  <Tooltip 
                    ...
                    formatter={(value: any, name: string) => {
                      if (name === 'parties') return [`${value} parties`, 'Nombre'];
                      return [value, name];
                    }}
                  />
                  ...
                  <div key={idx} className="bg-slate-950 p-3 rounded border border-slate-800">
                    <div className="flex justify-between items-center">
                      ...
                      <div className="text-right">
                        <div className="text-sm font-bold text-purple-400">{opening.parties} parties</div>
          ...
          <TabsContent value="phases" className="mt-6">
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-semibold text-slate-300">Performance par Phase</h3>
              ...
              <div className="bg-gradient-to-r from-slate-950 to-slate-900 p-4 rounded-lg border border-slate-800">
                <h4 className="text-sm font-semibold text-slate-300 mb-2">Points Clés</h4>
                <ul className="space-y-2 text-sm text-slate-400">
                  <li className="flex items-center gap-2">
                    <div className="w-2 h-2 rounded-full bg-green-500"></div>
                    <span>Point fort : {phaseData.reduce((max, p) => p.performance > max.performance ? p : max).phase}</span>
                  </li>
                  <li className="flex items-center gap-2">
                    <div className="w-2 h-2 rounded-full bg-orange-500"></div>
                    <span>Moyenne générale : {Math.round(phaseData.reduce((sum, p) => sum + p.performance, 0) / phaseData.length)}%</span>
                  </li>
                  <li className="flex items-center gap-2">
                    <div className="w-2 h-2 rounded-full bg-blue-500"></div>
                    <span>Style dominant : {stats.style}</span>
                  </li>
                </ul>
              </div>
            </div>
