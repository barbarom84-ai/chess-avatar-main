...
export function useStockfish() {
  ...
  const getBestMove = (
    ...
    const arenaStyle = options?.arenaStyle === true;
    const engineOpts = arenaStyle
      ? engineOptionsForArena(config)
      : engineOptionsForConfig(config);
    const skill = engineOpts.skill;
    const hashMb = 64;
    ...
    const playWithStockfish = () =>
      stockfishClient.enqueue<string>((ctx) => {
          ...
          if (line.startsWith("bestmove")) {
            ...
            if (multiPv > 1 && move) {
              move = humanBlunder
                ...
                : pickPersonaBiasedMove(move, lineMoves, config, Math.random, arenaStyle);
            }
            resetEngineOptions();
            return move;
          }
        ...
        if (arenaStyle || config.difficulty <= 3) {
          ctx.send("setoption name UCI_LimitStrength value true");
          ctx.send(`setoption name UCI_Elo value ${engineOpts.uciElo}`);
        } else {
          ctx.send("setoption name UCI_LimitStrength value false");
        }
        if (multiPv > 1) {
        ...
        ctx.send(
          `go depth ${arenaStyle ? engineOpts.depth : searchLimits.depth} movetime ${arenaStyle ? engineOpts.movetimeMs : searchLimits.movetime}`
        );
        setTimeout(() => ctx.stop(), (arenaStyle ? engineOpts.movetimeMs : searchLimits.movetime) + 5000);
      });
